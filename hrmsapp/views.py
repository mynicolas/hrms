...
def renderMacs(request):
    ...
    for mac in macs:
        if not mac.instance:
            tmp = {'mac': mac, 'instanceName': mac.instance[0].instanceName}
        else:
            tmp = {'mac': mac, 'instanceName': None}
        sendContent.append(tmp)
    return HttpResponse(josn.dumps(sendContent),
